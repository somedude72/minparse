...
parser.config().optional_args = {
    ...
    "include"    : (parser.STR, None, "--include", "Search only files matching GLOB (e.g., --include='*.py')"), 
...
def main():
    ...
    if parser.result().optional_args["help"]:
        print(parser.result().generated_help)
    else:
        pprint(parser.result().positional_args)
        pprint(parser.result().optional_args)
